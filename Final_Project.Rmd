...
crime <- read.csv("google_trends_crime.csv")
...
daily <- read.csv("gt_report_daily.csv") |> select(date, search, search_adj) |>  mutate(date=as.Date(date))
```

```{r}
filtered_topics <- document_topics |> 
  select(date, channel, duration, Topic1, Topic3, Topic13) |> group_by(date) |> 
  summarize(duration=sum(duration), Topic1and3=sum(Topic1 * duration + Topic3 * duration), Topic13=sum(Topic13 * duration))
```
...
```{r}
  ...
  filtered_topics$bush <- 
    ifelse(filtered_topics$president == "bush", 1, 0) 
  filtered_topics$obama <- 
    ifelse(filtered_topics$president == "obama", 1, 0) 
  filtered_topics$trump <- 
    ifelse(filtered_topics$president == "trump", 1, 0) 


fit.coverage <- lm(search_adj~duration+Topic1and3+Topic13+trump+date, data=filtered_topics)
